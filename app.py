...
# Streamlit интерфейс
st.title("LLM Chat Application with File Upload")
...
if st.button("Submit"):
    if query:
        ...
        if chroma_results and "documents" in chroma_results and chroma_results["documents"]:
            # Преобразуем каждый результат в строку, если он не строка
            context_list = [str(doc) if isinstance(doc, list) else doc for doc in chroma_results["documents"]]
            context = "\n\n".join(context_list)
            st.write("Context from Documents:")
            st.write(context[:500] + ("..." if len(context) > 500 else ""))  # Показываем до 500 символов текста

            # Формируем вопрос с учётом контекста
...
# Кнопка для отображения истории чата
